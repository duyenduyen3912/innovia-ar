...
export default function ProductDetail() {
    ...
    const severImages: string[] = data?.image.split(";")
    const image: string[] = severImages || [];
    const onHandleAddtocart = () => {
        if(ApiUser.getIdUser()) {
             addProductMutation.mutate({
                iduser: ApiUser.getIdUser(),
                idproduct : parseInt(id.toString(), 10),
                quantity : quantity,
                note: ''
            })
        } else {
    ...
    return (
        <>
            ...
            <Row justify={"center"} className={cx("product-detail")} gutter={40} >
                ...
                <Col  xs={24} lg={8} md={24} className="gutter-row">
                    ...
                    <div className={cx("product-price")}>
                        ...
                        <span className={cx("price")}>{formatCurrency(parseInt(data?.price, 10))} VNĐ</span>

                    </div>
                    <div className={cx("product-rate")}>
                        <Rate allowHalf disabled value={Math.round( data?.star * 2) / 2} style={{ color: "#a58838", fontSize: "12px" }} />
                    ...
                    <div className={cx("product-infor")}>
                        ...
                        <div className={cx("infor-wrap")}>
                            ...
                            <span className={cx("infor-text")}>{data?.category_name}</span>
                        </div>
                        <div className={cx("infor-wrap")}>
                            <span className={cx("infor-title")}>Loại:</span>
                            <span className={cx("infor-text")}>{data?.tag}</span>
                    ...
                    <div className={cx("product-order")}>
                        ...
                        { data?.model && <Button onClick={onHandleOpenModel} className={cx("order-btn", "try-btn")}>Dùng thử</Button> }
                ...
                <Collapse expandIcon={customExpandIcon} className={cx("product-description")}>
                    ...
                    <Collapse.Panel header={<CustomHeader>Thông tin sản phẩm</CustomHeader>} key="2" className={cx("description")}>
                        <div className={cx("infor-wrap")}>
                            ...
                            <span className={cx("infor-text")}>	{data?.weight}</span>
                        </div>
                        <div className={cx("infor-wrap")}>
                            <span className={cx("infor-title")}>Kích cỡ:</span>
                            <span className={cx("infor-text")}>{data?.size}</span>
                        </div>
                    </Collapse.Panel>
            ...
            <RecommendProduct category={data?.category_name} id = {data?.id} />
